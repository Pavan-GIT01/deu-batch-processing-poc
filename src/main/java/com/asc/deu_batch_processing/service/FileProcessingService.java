...
import com.asc.deu_batch_processing.repository.BatchRepository;
...
public class FileProcessingService {
    ...
    public void processMessage(String sqsMessage, String queueType) throws Exception {
        ...
        try {
            ...
            List<BatchRecord> records = parseRecords(json,key,queueType);
            ...
            if("BATCH".equalsIgnoreCase(queueType)) {
                for (int startIndex = 0; startIndex < records.size(); startIndex += batchSize) {
                    ...
                    List<BatchRecord> batchRecords = records.subList(startIndex, endIndex);
                    repository.saveAll(batchRecords);
    ...
    private Optional<String> extractS3Key(JsonNode event) {
        return Optional.ofNullable(event.path(BatchProcessConstant.RECORDS))
                ...
                .map(arr -> arr.get(0))
                .map(r -> r.path(BatchProcessConstant.S3)
    ...
    //parse JSON array
    private List<BatchRecord> parseRecords(String json,
                                           String sourceFile, String queueType) throws IOException {
    ...
    private Optional<BatchRecord> toBatchRecord(JsonNode node,
                                                String sourceFile, String queueType) {
